import { getSession, onboardingStatus, completeOnboarding, authenticatedFetch } from "./auth.js";
...
(async function bootstrap() {
  ...
  $("#email-continue").onclick = async () => {
    ...
    try {
      const response = await authenticatedFetch("/api/gmail/connect");
  ...
  $("#csv-file").onchange = async (event) => {
    ...
    try {
      const response = await authenticatedFetch("/api/imports/csv/inspect", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ csv_text: csvText, file_name: fileName }) });
  ...
  csvButton.onclick = async () => {
    ...
    try {
      const response = await authenticatedFetch("/api/imports/csv/commit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ csv_text: csvText, file_name: fileName }) });
