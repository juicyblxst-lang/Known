...
export async function getSession(){const callbackSession=await handleAuthCallback();if(callbackSession)return callbackSession;const accessToken=localStorage.getItem("known.access_token");if(!accessToken)return null;try{const user=await fetchUser(accessToken);if(user)return {accessToken,user};}catch{}return refreshSession();}
