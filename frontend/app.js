...
function escapeDate(date = new Date()) {
...
function initials(name) {
  return (name || "K").trim().split(/\s+/).slice(0, 2).map((part) => part[0]).join("").toUpperCase() || "K";
}
...
function setConnection(connected, text = "Connected") {
...
function renderOverview() {
  ...
  const now = new Date();
  $("#today-label").textContent = escapeDate(now).toUpperCase();
  $("#greeting").textContent = `${getGreeting(now.getHours())}, ${name}.`;
  ...
  $("#stat-import").textContent = customers.length ? "Complete" : "—";
  $("#stat-import-detail").textContent = customers.length ? `${formatCount(customers.length)} customer records` : "No import recorded";
  ...
  activity.innerHTML = customers.length
    ? `<div class="activity-event"><span class="activity-dot">✣</span><div><strong>Customer history imported</strong><p>${formatCount(customers.length)} customer records are available in Known. Select Customers to browse the imported history.</p><span class="activity-time">Available now</span></div></div>`
...
function renderCustomerDirectory(list = customers) {
  ...
  list.forEach((customer) => {
    ...
    button.querySelector("small").textContent = [customer.tier, customer.email].filter(Boolean).join(" · ") || "Customer record";
...
function renderCustomerDetail(data) {
  ...
  $("#detail-meta").textContent = data.customer ? [data.customer.tier, data.customer.email].filter(Boolean).join(" · ") : "Customer information will appear here.";
  ...
  if (!selectedOrders.length) {
    orderBody.innerHTML = `<div class="empty-state">No order history in this customer record.</div>`;
  } else {
    selectedOrders.forEach((order) => {
      const row = document.createElement("div");
      row.className = "order";
      const statusClass = order.status === "delivered" ? "delivered" : "delayed";
      row.innerHTML = `<b></b><span class="status ${statusClass}"></span><small></small>`;
      row.querySelector("b").textContent = order.id || "Order";
      row.querySelector(".status").textContent = order.status || "unknown";
      row.querySelector("small").textContent = `$${Number(order.total || 0).toFixed(2)} · ${(order.items || []).join(", ") || "No items listed"}`;
  ...
  if (!memories.length) {
    memoryBody.innerHTML = `<div class="empty-state">No relevant memory has been surfaced yet.</div>`;
  } else {
  ...
  renderCustomerDirectory($$(".directory-row").length ? getFilteredCustomers() : customers);
}

function getFilteredCustomers() {
  const query = $("#customer-search")?.value.trim().toLowerCase() || "";
  if (!query) return customers;
  return customers.filter((customer) => [customer.name, customer.email, customer.tier].filter(Boolean).some((value) => String(value).toLowerCase().includes(query)));
}
...
function setupSettings() {
  ...
  importToggle.addEventListener("change", () => localStorage.setItem("known.notify.import", String(importToggle.checked)));
  conversationToggle.addEventListener("change", () => localStorage.setItem("known.notify.conversations", String(conversationToggle.checked)));
}

async function loadConversation(customer, newSession = false) {
  if (newSession) {
    sessionId = null;
    $("#conversation-session").textContent = "New session";
  ...
  if (!id) {
    $("#conversation-session").textContent = "No conversation yet";
    $("#messages").innerHTML = `<div class="empty-state">No conversation loaded.</div>`;
    return;
  }
  ...
  $("#conversation-session").textContent = `Session: ${data.persistence}`;
...
function renderConversation(items) {
  $("#messages").innerHTML = "";
  items.forEach((item) => addMessage(item.role === "assistant" ? "KNOWN" : selectedCustomer.name.toUpperCase(), item.content, item.role === "assistant" ? "agent-msg" : "customer-msg"));
}
...
async function sendMessage(message) {
  const response = await authenticatedFetch("/api/support", {
    ...
    body: JSON.stringify({ customer_id: selectedCustomer.id, message, conversation_id: sessionId }),
  });
  if (!response) return;
...
function setupConversation() {
  ...
  $("#composer").addEventListener("submit", async (event) => {
    ...
    try { await sendMessage(message); } catch (error) { addMessage("SYSTEM", error.message || "Unable to reach Known.", "agent-msg"); } finally { button.disabled = false; input.focus(); }
  });
}

async function openConversation() {
  if (!selectedCustomer) return;
  $("#conversation-title").textContent = selectedCustomer.name;
  $("#conversation-meta").textContent = [selectedCustomer.tier, selectedCustomer.email].filter(Boolean).join(" · ");
...
async function bootstrap() {
  try {
    ...
    // Customer details remain in the directory view; the conversation view is intentionally separate.
    $("#detail-name").addEventListener("dblclick", openConversation);
  } catch (error) {
