...
async function selectCustomer(customer, preferredSessionId = null) {
  const response = await authenticatedFetch(`/api/workspace/${encodeURIComponent(customer.id)}`); if (!response) return;
  if (!response.ok) { $("#detail-meta").textContent = "Unable to load this customer right now."; return; }
  const data = await response.json(); renderCustomerDetail(data); if (preferredSessionId) { sessionId = preferredSessionId; localStorage.setItem(`known.session.${customer.id}`, preferredSessionId); } switchView("customers");
}
...
async function loadConversation(customer, newSession = false) {
  ...
  const id = sessionId || localStorage.getItem(`known.session.${customer.id}`); if (!id) { $("#conversation-session").textContent = "No conversation yet"; $("#messages").innerHTML = `<div class="empty-state">No conversation loaded.</div>`; return; }
  const response = await authenticatedFetch(`/api/sessions/${encodeURIComponent(id)}?customer_id=${encodeURIComponent(customer.id)}`); if (!response || !response.ok) return; const data = await response.json(); sessionId = data.session_id; $("#conversation-session").textContent = `Session: ${data.persistence}`; renderConversation(data.messages || []);
}
...
async function bootstrap() { try { bindNavigation(); setupSettings(); setupConversation(); session = await getSession(); if (!session) { location.href = "./login.html"; return; } setupProfile(); setConnection(true); await loadCustomers(); $("#customer-search").addEventListener("input", () => renderCustomerDirectory(getFilteredCustomers())); $("#detail-name").addEventListener("dblclick", openConversation); } catch (error) { setConnection(false, "Unavailable"); $("#recent-activity").innerHTML = `<div class="empty-state">${error.message || "Unable to initialize Known."}</div>`; } }
bootstrap();
