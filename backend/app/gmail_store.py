...
class GmailConnectionStore:
    ...
    def upsert(self,business_id:str,data:dict):
        payload={**data,'business_id':business_id};r=httpx.post(f'{self.url}/rest/v1/gmail_connections',params={'on_conflict':'business_id'},headers={**self._headers(),'Prefer':'resolution=merge-duplicates,return=representation'},json=payload,timeout=10);r.raise_for_status();rows=r.json();return rows[0] if rows else None
    ...
    def list_business_ids(self):
        r=httpx.get(f'{self.url}/rest/v1/gmail_connections',params={'select':'business_id'},headers=self._headers(),timeout=10);r.raise_for_status();return [row['business_id'] for row in r.json()]
