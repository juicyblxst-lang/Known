...
def create_oauth_state(business_id: str, user_id: str, shop_domain: str) -> str:
    ...
    _db_write("shopify_oauth_states", {
        "nonce": nonce,
...
def _page_query(shop_domain: str, token: str, query: str, root: str) -> list[dict[str, Any]]:
    ...
    while True:
        data = _graphql(shop_domain, token, query, {"cursor": cursor})
...
query Customers($cursor: String) {
  customers(first: 100, after: $cursor) {
    nodes { id firstName lastName displayName email }
...
query Orders($cursor: String) {
  orders(first: 100, after: $cursor, sortKey: UPDATED_AT) {
    nodes {
      id name updatedAt createdAt displayFinancialStatus displayFulfillmentStatus totalPriceSet { shopMoney { amount currencyCode } }
      customer { id }
...
def sync_shop(business_id: str, shop_domain: str) -> dict[str, int]:
    ...
    for order in orders:
        ...
        _db_write("orders", {
            "id": _stable_order_id(shop_domain, sid),
            "business_id": business_id,
            "customer_id": customer_id,
            "status": (order.get("displayFulfillmentStatus") or order.get("displayFinancialStatus") or "unknown").lower(),
            "total": float(money.get("amount") or 0),
            "items": items,
            "created_at": order.get("createdAt") or utcnow().isoformat(),
        }, conflict="id")
...
def save_installation(business_id: str, shop_domain: str, token_data: dict[str, Any]) -> None:
    ...
    if token_data.get("expires_in"):
        expires_at = (utcnow() + timedelta(seconds=int(token_data["expires_in"]))).isoformat()
    shop = _graphql(shop_domain, token_data["access_token"], SHOP_QUERY)["shop"]
    _db_write("shopify_installations", {
        "business_id": business_id,
        "shop_domain": shop_domain,
        "shop_name": shop.get("name"),
        "access_token_encrypted": encrypt_token(token_data["access_token"]),
        "refresh_token_encrypted": encrypt_token(token_data["refresh_token"]) if token_data.get("refresh_token") else None,
        "access_token_expires_at": expires_at,
        "scopes": scopes,
        "updated_at": utcnow().isoformat(),
        "sync_status": "pending",
    }, conflict="business_id")
