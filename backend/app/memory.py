...
class SibylMemory:
    """Semantic Sibyl memory backed by durable customer memory in Supabase."""
    ...
    def _durable_health(self) -> bool:
        if not self.durable_configured: return False
        try:
    ...
    def _durable_request(self, method: str, **kwargs: Any) -> list[dict[str, Any]]:
        if not self.durable_configured: raise RuntimeError("Durable customer memory is not configured")
        headers = kwargs.pop("headers", None) or service_headers(self.supabase_key)
        response = httpx.request(method, f"{self.supabase_url}/rest/v1/customer_memories", headers=headers, timeout=10, **kwargs)
    ...
    def _close(client: MemoryClient) -> None:
        try: getattr(client.storage, "close", lambda: None)()
        except Exception: pass

    @staticmethod
    def _normalize_hit(hit: Any) -> dict[str, Any]:
        if not isinstance(hit, dict): return {"content": str(hit)}
        body = hit.get("body"); content = hit.get("content")
        if content is None:
            if isinstance(body, dict): content = body.get("content") or body.get("value") or str(body)
            elif body is not None: content = str(body)
        normalized = dict(hit)
        if content is not None: normalized["content"] = str(content)
        return normalized

    @staticmethod
    def _error_message(exc: Exception) -> str:
        if isinstance(exc, CapExceededError): return "Sibyl memory tier capacity exceeded"
        if isinstance(exc, TierGateError): return "Sibyl memory feature requires the configured tier"
        if isinstance(exc, TierVerificationError): return "Sibyl memory tier verification failed"
        if isinstance(exc, ValidationError): return f"Sibyl memory validation failed: {exc}"
        if isinstance(exc, NotFoundError): return "Sibyl memory entry not found"
        if isinstance(exc, SibylMemoryError): return str(exc)
        return str(exc)

    def search(self, business_id: str, customer_id: str, query: str, limit: int = 8) -> MemoryResult:
        if not query or len(query.strip()) < 3: return MemoryResult([], True)
        client = None
        try:
    ...
    def import_customer_history(self, business_id: str, customers: list[dict[str, Any]], orders: list[dict[str, Any]]) -> dict[str, int]:
        ...
        for order in orders: orders_by_customer.setdefault(str(order.get("customer_id", "")), []).append(order)
        memories_written = 0
        for customer in customers:
            customer_id = str(customer["id"]); customer_orders = orders_by_customer.get(customer_id, [])
            ...
            ok, error = self.remember(business_id, customer_id, "\n".join(lines), "customer_history")
