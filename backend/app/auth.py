...
async def _provision_business(url: str, service_key: str, user: dict) -> str:
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        membership = await client.get(f"{url}/rest/v1/business_memberships", params={"user_id": f"eq.{user_id}", "select": "business_id", "limit": "1"}, headers=headers)
        ...
        created = await client.post(f"{url}/rest/v1/businesses", headers={**headers, "Prefer": "return=representation"}, json={"id": business_id, "name": f"{local or 'My'} workspace"})
        created.raise_for_status()
        membership = await client.post(f"{url}/rest/v1/business_memberships", headers={**headers, "Prefer": "return=representation"}, json={"business_id": business_id, "user_id": user_id, "role": "owner"})
        membership.raise_for_status()
        updated_metadata = dict(user.get("app_metadata") or {})
        updated_metadata["business_id"] = business_id
        auth_update = await client.put(f"{url}/auth/v1/admin/users/{user_id}", headers=headers, json={"app_metadata": updated_metadata})
...
async def require_auth(authorization: str | None = Header(default=None)) -> AuthContext:
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            response = await client.get(f"{url}/auth/v1/user", headers={"apikey": service_key, "Authorization": f"Bearer {token}"})
