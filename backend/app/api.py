...
async def search_workspace(q: str = Query("", min_length=1, max_length=120), auth: AuthContext = Depends(require_auth)) -> dict[str, list[dict]]:
    try: return store.search(auth.business_id, q)
    except (httpx.HTTPError, ValueError): raise upstream_error()
...
async def commit_csv(request: CSVImportRequest, auth: AuthContext = Depends(require_auth)) -> dict[str, object]:
    try:
        ...
        if memory_counts["memories"] != len(result["customers"]): raise RuntimeError("Customer memory could not be fully initialized")
...
class ShopifyConnectRequest(BaseModel): shop_domain: str

@router.get("/shopify/status")
async def shopify_status(auth: AuthContext = Depends(require_auth)) -> dict:
    connected = installation(auth.business_id); return {"connected": bool(connected), "installation": connected}
...
async def shopify_callback(request: Request) -> RedirectResponse:
    frontend=os.getenv("KNOWN_PUBLIC_URL","/").rstrip("/") or "/"
    try:
        params={key:value for key,value in request.query_params.items()}; shop_domain=validate_shop_domain(params.get("shop",""))
        if not verify_oauth_hmac(params): raise HTTPException(status_code=401,detail="Invalid Shopify OAuth signature")
        if params.get("error"): raise HTTPException(status_code=400,detail="Shopify authorization was not completed")
        code,state=params.get("code"),params.get("state")
        if not code or not state: raise HTTPException(status_code=400,detail="Missing Shopify OAuth parameters")
        state_data=consume_oauth_state(state,shop_domain); token_data=exchange_code(shop_domain,code); save_installation(state_data["business_id"],shop_domain,token_data)
        try: register_webhooks(shop_domain,token_data["access_token"])
        except Exception as exc: logger.warning("Shopify webhook registration skipped after successful connection: %s",exc)
        sync_shop(state_data["business_id"],shop_domain); return RedirectResponse(url=f"{frontend}/?shopify=connected",status_code=303)
    except HTTPException as exc: logger.warning("Shopify OAuth failed: %s",exc.detail)
    except Exception: logger.exception("Shopify OAuth callback failed")
...
async def shopify_webhook(request: Request) -> dict[str,str]:
    ...
    if not verify_webhook(body,request.headers.get("X-Shopify-Hmac-Sha256")): raise HTTPException(status_code=401,detail="Invalid Shopify webhook signature")
    shop=request.headers.get("X-Shopify-Shop-Domain","").lower(); topic=request.headers.get("X-Shopify-Topic",""); webhook_id=request.headers.get("X-Shopify-Webhook-Id","")
    if not shop or not topic: raise HTTPException(status_code=400,detail="Missing Shopify webhook headers")
    ...
    if not current: webhook_fail(webhook_id,"No active Known installation for Shopify store"); return {"status":"ignored"}
    try: sync_shop(current["business_id"],shop); webhook_complete(webhook_id)
