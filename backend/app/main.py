...
async def support(request: SupportRequest, session_id: str | None = None, auth: AuthContext = Depends(require_auth)) -> SupportSessionResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="Customer data service unavailable") from exc
    ...
    if durable_sessions.configured:
        ...
        try:
            session = local_sessions.get_or_create(resolved_session_id, customer_id)
        except ValueError as exc:
            raise HTTPException(status_code=403, detail="session does not belong to customer") from exc
    ...
    context_request = SupportContextRequest(customer=customer_data, message=request.message, conversation=conversation, orders=orders_data)
