...
def ready() -> dict[str, object]:
    ...
    checks={"frontend":(Path(__file__).resolve().parents[2]/"frontend").exists(),"supabase":store.configured and durable_sessions.configured,"openai":bool(os.getenv("OPENAI_API_KEY")),"sibyl":bool(sibyl.get("configured"))}
...
async def support(request: SupportRequest,session_id:str|None=None,auth:AuthContext=Depends(require_auth))->SupportSessionResponse:
    ...
    try:
        session=durable_sessions.get_or_create(resolved_session_id,request.customer_id,auth.business_id)
