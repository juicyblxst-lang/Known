...
def _clean(value: Any) -> str: return str(value or "").strip()

def import_csv(contents: bytes, business_id: str, store: StructuredStore) -> dict[str, int]:
    reader = csv.DictReader(io.StringIO(contents.decode("utf-8-sig"))); fields = {f.strip() for f in (reader.fieldnames or []) if f}
    missing = sorted(REQUIRED - fields)
    if missing: raise ValueError(f"CSV missing required columns: {', '.join(missing)}")
    customers: dict[str, dict[str, Any]] = {}; orders: dict[str, dict[str, Any]] = {}; row_count = 0
    for raw in reader:
        row_count += 1; cid, oid, email = _clean(raw.get("customer_id")), _clean(raw.get("order_id")), _clean(raw.get("email")).lower()
        ...
        item, variant = _clean(raw.get("product_name")), _clean(raw.get("product_variant")); item = f"{item} ({variant})" if variant and item else (variant or item)
        order = orders.setdefault(oid, {"id": oid, "business_id": business_id, "customer_id": cid, "status": "unknown", "total": 0, "items": [], "order_date": None, "fulfillment_status": None, "currency": None, "shipping_city": None, "shipping_country": None, "product_variant": variant or None})
        order.update({"status": _clean(raw.get("order_status")) or order["status"], "order_date": _clean(raw.get("order_date")) or None, "fulfillment_status": _clean(raw.get("fulfillment_status")) or None, "currency": _clean(raw.get("currency")) or None, "shipping_city": _clean(raw.get("shipping_city")) or None, "shipping_country": _clean(raw.get("shipping_country")) or None, "product_variant": variant or None})
